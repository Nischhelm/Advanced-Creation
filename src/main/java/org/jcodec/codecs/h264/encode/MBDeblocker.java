...
public class MBDeblocker {
    ...
    static int calcQpChroma(int qp, int crQpOffset) {
        return QP_SCALE_CR[clip(qp + crQpOffset, 0, 51)];
    }
    ...
    public void deblockMBGeneric(EncodedMB curMB, EncodedMB leftMB, EncodedMB topMB, int[][] vertStrength,
        ...
        if (leftMB != null) {
            ...
            int avgQp = clip((leftMB.getQp() + curMB.getQp() + 1) >> 1, 0, 51);
            int avgChQp = clip((leftChQp + curChQp + 1) >> 1, 0, 51);
        ...
        if (topMB != null) {
            ...
            int avgQp = clip((topMB.getQp() + curMB.getQp() + 1) >> 1, 0, 51);
            int avgChQp = clip((topChQp + curChQp + 1) >> 1, 0, 51);
