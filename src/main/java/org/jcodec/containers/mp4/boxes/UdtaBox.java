...
public class UdtaBox extends NodeBox {
    ...
    public MetaBox meta() {
        return findFirst(this, MetaBox.class, MetaBox.fourcc());
    }
    ...
    public List<MetaValue> getUserDataString(String id) {
        LeafBox leafBox = findFirst(this, LeafBox.class, id);
        if (leafBox == null)
            return null;
    ...
    public void setUserDataString(String id, List<MetaValue> data) {
        ...
        LeafBox box = createLeafBox(Header.createHeader(id, bb.remaining() + 4), bb);
        replaceBox(box);
    }
    ...
    public void setMetadata(Map<Integer, MetaValue> udata) {
        ...
        for (Entry<Integer, MetaValue> entry : entrySet) {
            ...
            LeafBox box;
            try {
                box = createLeafBox(Header.createHeader(new String(bytes, "iso8859-1"), bb.remaining() + 4),
                        bb);
                replaceBox(box);
            } catch (UnsupportedEncodingException e) {
            }
