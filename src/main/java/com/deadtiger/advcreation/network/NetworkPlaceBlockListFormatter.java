            ...
            if(allThere)
            {
                ...
                if(BLOCKS_TO_CHECK_FOR_PLACEMENT.isEmpty())
                {
                    ...
                    if(AdvCreation.getMode() == EnumMainMode.PLACE)
                    {
                        ...
                        if( time - ((SelectTemplateGuiOverlay) GuiOverlayManager.TEMPLATE_SELECTION).lastTextUpdate > ((SelectTemplateGuiOverlay) GuiOverlayManager.TEMPLATE_SELECTION).textUpdateRate)
                        {
                            ...
                            ((SelectTemplateGuiOverlay) GuiOverlayManager.TEMPLATE_SELECTION).holdMessage = false;

                    }
                    else
                    {
                        long time = System.currentTimeMillis();
                        if( time - ((SelectInventoryItemGuiOverlay) GuiOverlayManager.INVENTORY_SELECTION).lastTextUpdate > ((SelectInventoryItemGuiOverlay) GuiOverlayManager.INVENTORY_SELECTION).textUpdateRate)
                        {
                            ...
                            ((SelectInventoryItemGuiOverlay) GuiOverlayManager.INVENTORY_SELECTION).holdMessage = false;
                    }
